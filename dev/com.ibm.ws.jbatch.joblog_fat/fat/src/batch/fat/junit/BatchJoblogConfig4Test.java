...
public class BatchJoblogConfig4Test {

    private static final String appName = "batchFAT";
    private static final String SUCCESS_MESSAGE = "TEST PASSED";

    private static final LibertyServer server = LibertyServerFactory.getLibertyServer("com.ibm.ws.jbatch.joblog.config4.fat");
    ...
    public void testBasicJobLogging() throws Exception {
        ...
        assertFalse("Should not find batchlet trace in server log", BatchJoblogFatUtils.findBatchletAllTrace(server, server.getMostRecentTraceFile()));
        ...
        // verify internal batch JobLogger wrote to the job log but NOT the server log
        assertTrue("Should find JobLogger trace in job log", BatchJoblogFatUtils.findJobLoggerFineTrace(server, logFile));
        assertFalse("Should not find JobLogger trace in server log", BatchJoblogFatUtils.findJobLoggerFineTrace(server, server.getMostRecentTraceFile()));

        // verify BatchKernelImpl trace wrote to the server log but NOT the job log.
        // Note: include spaces to ensure we match BatchKernelImpl trace records.
        assertFalse("Should not find BatchKernelImpl trace in job log",
    ...
    private static void log(String method, Object msg) {
        Log.info(BatchJoblogConfig4Test.class, method, String.valueOf(msg));
    }
